...
def main():
    ...
    help = []
    xt = []
    def dino2(a): # đưa số đạn lên màn hình
    ...
    while run:
        ...
        if userInput[pygame.K_h]  :#and len(xt)==0:
            if len(help) == 0:
                help.append(1)

        if len(help) == 1:
            if sti == 7:
                SCREEN.blit(TANK[7], (80, 200))
            else:
        ...
        ty = 0
        laser = Laser()
        if oke == True and player.dino_jump == False: # khi không nhảy mới có thể bắn được
            if dem == 0:
                if len(ls1) <= 20:
                    ls.append(laser)
                    ls1.append(1)
            ...
            for i in ls:
                if i.rect.x < SCREEN_WIDTH:
                    ...
                    if len(obstacles) != 0 and len(ob) != 0: 
                        if check(obstacles[0], i) == True:# đạn trúng vật cản
                            s1 = obstacles[0].getx()
                            s2 = obstacles[0].gety()
                            ty = obstacles[0].gett()
                            obstacles = []
                            ...
                            if len(dt) == 0:
                                ...
                                destr.set(s1)
                                destr.sety(s2)
                                dt.append(destr) # set vật bị phá hủy
